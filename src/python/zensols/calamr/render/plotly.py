...
class PlotlyGraphRenderer(GraphRenderer):
    ...
    def _add_comp(self, comp: DocumentGraphComponent, z: int) -> _CompPlot:
        ...
        comp: DocumentGraphComponent = comp.detach()
        graph: Graph = comp.graph
        layt: Layout = graph.layout('rt')
        # flip along x-axis since the tree is drawn upside down, otherwise the
        # sentences are rendered in the reverse order
        if 0:
            if comp.name == 'summary':
                layt.rotate(90)
        else:
            layt.mirror(dim=0)
        ...
        for i in range(n_nodes):
            ...
            lb: str = f'{gn}[{i}]' if add_id else str(gn)
        ...
        coords = dict(map(
            lambda e: (e, ((*layt[e.source], z), (*layt[e.target], z))),
            graph.es))
        return _CompPlot(
            comp,
            node_trace,
    ...
    def _render_to_file(self, context: RenderContext, out_file: Path):
        ...
        comp_graph: DocumentGraph
        if doc_graph.original_graph is not None:
            comp_graph = doc_graph.original_graph
        else:
            comp_graph = doc_graph
        cix: int
        comps: List[DocumentGraphComponent] = sorted(
            comp_graph.components,
            key=lambda c: c.name,
