...
class DocumentNode(GraphNode):
    ...
    :class:`~zensols.amr.FeatureDocument` as root node.  This class represents
    nodes in a graph that either:
    ...
    """The owning feature document containing all sentences/tokensof the graph.
...
class GraphEdge(GraphAttribute, metaclass=ABCMeta):
    ...
    *Implementation note*: It seems :mod:`igraph` can only handle large values
    ...
    def __post_init__(self):
        super().__post_init__()
        self._value_str = PersistedWork('_value_str', self)
        self._label = PersistedWork('_label', self)
